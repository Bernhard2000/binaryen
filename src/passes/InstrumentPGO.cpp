...
// Global counter for the number of instrumented blocks
static Index nextCounterIndex = 0;
...
private:
  void ensureProfileDataGlobal(Module* module) {
    ...
    // Create a global for the profile data (i32 array)
    // We'll use a mutable global that points to memory
    auto* memory = module->getMemoryOrNull();
    if (!memory) {
      // If no memory exists, create one
      memory = new Memory();
      memory->initial = 1; // 1 page (64KB)
      memory->max = 0; // No maximum
      module->addMemory(memory);
    }
    
    // Create the profile data global (i32 pointer to memory)
  ...
  void instrumentFunction(Function* func, Module* module) {
    ...
    std::vector<Expression*> originalBlocks;
